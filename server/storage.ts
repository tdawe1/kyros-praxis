...
export class MemStorage implements IStorage {
    ...
    const sampleRuns: Run[] = [
      {
        ...
        completedAt: new Date(Date.now() - 8000).toISOString(), // 8 seconds ago
        duration: 134, // 2m 14s
        agentId: "planner",
      },
      {
      ...
      },
      {
        ...
        completedAt: new Date(Date.now() - 10 * 60 * 1000 + 15000).toISOString(),
        duration: 105, // 1m 45s
        agentId: "planner",
      },
    ];
  ...
  async getRecentRuns(limit = 10): Promise<RunWithAgent[]> {
    ...
    return runs.map(run => ({
      ...run,
      agentName: run.agentId ? this.agents.get(run.agentId)?.name : undefined,
    }));
  }
