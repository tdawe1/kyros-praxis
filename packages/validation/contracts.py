...
class OutputValidator:
    ...
    async def validate_against_dod(self, artifacts:list[dict], dod:list[str])->ValidationResult:
        ...
        return ValidationResult(ok=(len(missing)==0), findings=[f"Missing DoD: {m}" for m in missing])
    async def validate_security(self, path:str)->ValidationResult: return ValidationResult(ok=True)