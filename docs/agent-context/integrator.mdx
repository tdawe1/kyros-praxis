...
## Integrator Merge Step (Proposed)

- Acquire lease: mark an `integrate` lease in `collaboration/state/locks.json` to avoid parallel merges.
- Prep workspace: `git fetch --all --prune && git checkout main && git pull`.
- Validate PR branch locally:
  - `gh pr checkout <PR#>` or `git checkout <branch>`
  - Run: `scripts/check.sh --base main --run-tests` (must pass)
- Merge protocol:
  - Enable rerere (optional): `git config rerere.enabled true`
  - Rebase onto `main` (preferred) or merge; resolve conflicts
  - Conflict resolution checklist: preserve schema changes, re-run `scripts/check.sh`, emit events (see below)
- Events during merge:
  - Append to `collaboration/events/events.jsonl`:
    - `merge_started`, `merge_conflict` (per file, include resolver + strategy), `merge_resolved`, `merge_pushed`
- Finalize:
  - Push: `git push` (for rebase, with `--force-with-lease` when necessary)
  - Confirm CI green; update task status to `merging` → `done` in `collaboration/state/tasks.json`
  - Comment on PR summarizing resolution and follow-ups
